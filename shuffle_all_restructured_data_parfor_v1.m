...
%% Loading bat details
bat_name = {'Adren','Kong'};
dates{1} = {'181010','181012','181013','181015','181020','181021'};
dates{2} = {'181009','181010','181018_1','181019'};
spike_thresh = 40;
%% Parameters
tic
HS_shuff_arr = [];
...
%% Main loop
for bat_index=1:size(bat_name,2)
    bat_folder_name = strcat('D:\Yartsev lab\Height cell experiment\restructured data\',bat_name{bat_index});
    for date_index = 1:size(dates{bat_index},2)
        bat_folder_name_date = strcat(bat_folder_name,filesep,dates{bat_index}{date_index});
        cd(bat_folder_name_date)
        data_file_name = strcat('data_plane_cell_',bat_name{bat_index},'_',dates{bat_index}{date_index},'.mat');
        load(data_file_name)
        shuffled_data_plane_cell = [];
        for channel_index = 1:size(data_plane_cell,2)
            ...
            if numel(data_plane_cell{channel_index}) ~= 0
                x_video = data_plane_cell{channel_index}.x_video_FE;
                y_video = data_plane_cell{channel_index}.y_video_FE;
                z_video = data_plane_cell{channel_index}.z_video_FE;
                x_spikes = data_plane_cell{channel_index}.x_spikes_FE;
                y_spikes = data_plane_cell{channel_index}.y_spikes_FE;
                z_spikes = data_plane_cell{channel_index}.z_spikes_FE;
                pos = [x_video y_video z_video];
                firpos_original = [x_spikes y_spikes z_spikes];
                
                parfor shuffle_index = 1:nshuffle
                    fprintf('\n Shuffle number: %d/%d, Bat name: %s, Date: %s, Channel_number: %d/%d ',shuffle_index,nshuffle,bat_name{bat_index},dates{bat_index}{date_index},channel_index,size(data_plane_cell,2))
                    ...
                    x_spikes_shuffle = x_vidfeo_for_plotting(ind_shuff);
            ...
            shuffled_data_plane_cell{channel_index}.SI3D = SI3D_shuff_arr;
            shuffled_data_plane_cell{channel_index}.SIXY = SI_XY_shuff_arr;
            shuffled_data_plane_cell{channel_index}.SIZY = SI_ZY_shuff_arr;
            shuffled_data_plane_cell{channel_index}.SIXZ = SI_XZ_shuff_arr;
            shuffled_data_plane_cell{channel_index}.SIprj = SI_prj_shuff_arr;
            shuffled_data_plane_cell{channel_index}.PI = PI_shuff_arr;
        end        
        cd(bat_folder_name_date)
        shuffled_data_filename = strcat('shuffled_data_plane_cell_',bat_name{bat_index},'_',dates{bat_index}{date_index});
        save(shuffled_data_filename,'shuffled_data_plane_cell','-v7.3')
    end
end
